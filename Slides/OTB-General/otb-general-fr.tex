...
\end{block}
\begin{block}{Utiliser Monteverdi2}
...
\column{0.6\textwidth}
\begin{itemize}
\item l'OTB a été utile à (certains) des utilisateurs ORFEO/RTU
...
\begin{columns}
  ...
  \begin{itemize}
    ...
    \item Projet SEAS (IRD)
    \item L'OTB est utilisée dans certains composant des segments sols S2 et Venus (CNES et ESA)
    \item Le programme de recherche TCM (ETS Quebec)
    \item Le projet de recherche FP7 Tolomeo (CESBIO)
...
\begin{block}{Encore!}
\begin{itemize}
\item Après ORFEO, le CNES va poursuivre le développement de l'OTB
\item De nouveaux défis arrivent avec des missions comme Sentinel2, SWOT ou le post-Pléiades
\item Le projet va se doter d'une gouvernance ouverte 
\end{itemize}
\end{block}

\begin{block}{Prochainement dans votre OTB}
\begin{itemize}
\item Un gros chantier de modularisation est en cours
\item L'OTB se compilera bientôt avec un superbuild,
\item Une nouvelle application de calcul de bandes (BandMathX) est déjà disponible!
\item Prochaine version prévue pour début 2015.
\end{itemize}
\end{block}
...
\begin{frame}
\frametitle{Demander du support / proposer son aide}
\vspace{-0.2cm}
\begin{block}{Ressources générales}
\vspace{-0.2cm}
\begin{description}
...
\vspace{-0.2cm}
\begin{description}
\item[Doxygen] http://www.orfeo-toolbox.org/doxygen/
\item[Guides] Software Guide (pour les utilisateurs C++) et CookBook (pour tous)
...
\vspace{-0.2cm}
\begin{description}
\item[Que se passe-t-il ?] scrum.orfeo-toolbox.org
\item[Quelle est la météo du projet?] dash.orfeo-toolbox.org
\item[Un coup d'oeil au code?] hg.orfeo-toolbox.org
\item[Vous avez trouvé un bug?] bugs.orfeo-toolbox.org
\end{description}
\end{block}
\end{frame}
