...
\subsubsection{Stereo reconstruction from VHR optical image pair}

This section describes how to convert pair of images into elevation information.

\subsubsection{Estimate epipolar geometry transformation from image pair}
The aim of this application is to Epipolar geometry is the geometry of stereo vision.
to generate resampling grids from raw images to epipolar geometry
...
After applying this transformation, it reduces the problem of elevation  (or stereo coresspondences determinantion  to a 1-D problem)

\href{http://en.wikipedia.org/wiki/Epipolar_geometry}

The  problem is a little bit more complicated in case of VHR optical images but (see \href{http://en.wikipedia.org/wiki/Epipolar_geometry#Epipolar_geometry_of_pushbroom_sensor})
But the

\begin{equation}
   (long,lat) = {f_{image_left}}^{forward)(i,j,h)
\end{equation}
and
\begin{equation}
   (long,lat,h) = {f_{image_left}}^{inverse)(i,j)
\end{equation}
Using jointly the forward and inverse function from the image pair, we can construct a colocalization function between a pixel in the first and in the second image.
...
\begin{equation}
(i_{right},j_{right}) = f_{left \rightarrow right} (i_{left} , j_{left} , h)
\end{equation}
\begin{equation}
f_{left \rightarrow right} (i_{left} , j_{left} , h) =  {f_{right}}^{inverse} (f_{left}}^{forward}((i_{left} , j_{left} ), h)
\end{equation}

The value of this function is not really important, the thing to understand is that if you are able to determine for a point on the left image, the corresponding pixel  in the right image, as you know the expression of  colocalization
function, you've got the information
You've got now the mathematical basis to understand how  3D information can be extracted by examination of the relative positions of objects in the two 2D images.

The application will not resample input images if epipolar geometry but produce two images which are \textgreater{deformation grid}
\begin{verbatim}
...
The estimated transformation is not analitical in a sense that you estimate a displacment for each pixel in the two input images. Despite that, you can see that the application can accept a ``step'' parameter to estimate displacments on a coarser grid. Here we estimate the displcments every 10 pixels.
 That's because in most cases with a pair of VHR and a small angle between the two images, this grid is almost regular. Moreover, in case of push-broom geometry, the algorithm use lots of memory. It is a good solution to
...
small parallaxes errors (i.e. epipolar lines exhibit a small shift in the vertical direction, around 1 pixel).
