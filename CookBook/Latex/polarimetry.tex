...
- channel #0 : \f$ 0.5*\textbf{Re}( Coherency[0]+Coherency[3]+Coherency[5]) \f$ \newline
- channel #1 : \f$ 0.5*\textbf{Re}( Coherency[0]+Coherency[3]-Coherency[5]) \f$ \newline
- channel #2 : \f$ 0.5*\textbf{Re}( Coherency[0]-Coherency[3]+Coherency[5]) \f$ \newline
- channel #3 : \f$ 0.5*\textbf{Re}(-Coherency[0]+Coherency[3]+Coherency[5]) \f$ \newline
- channel #4 : \f$ \textbf{Re}(Coherency[1]) \f$ \newline
- channel #5 : \f$ \textbf{Re}(Coherency[2]) \f$ \newline
- channel #6 : \f$ \textbf{Im}(Coherency[4]) \f$ \newline
- channel #7 : \f$ \textbf{Re}(Coherency[4]) \f$ \newline
- channel #8 : \f$ \textbf{Im}(Coherency[2]) \f$ \newline
- channel #9 : \f$ \textbf{Im}(Coherency[1]) \f$ \newline
...
- channel #1 : \f$ 0.25(C_{1}-C_{3}+2*C_{5}+2*C_{9} - 0.5\textbf{i}.(C_{4}+C_{7}+C_{8}) \f$ \newline
- channel #2 : \f$ 0.25(C_{1}+C_{3}-4*C_{2}-2*C_{6} - \textbf{i}.(C_{4}+C_{8}) \f$ \newline
- channel #3 : \f$ 0.25(C_{1}+C_{3}+2*C_{6} \f$ \newline
- channel #4 : \f$ 0.25(C_{1}-C_{3}+2*C_{5}-2*C_{9} - 0.5\textbf{i}.(C_{4}-C_{7}+C_{8}) \f$ \newline
- channel #5 : \f$ 0.25(C_{1}+C_{3}+4*C_{2}-2*C_{6}+4*C_{5}+4*C_{9}) \f$ \newline

Where: \newline
- \f$ C_{1} = S_{hh}*S_{hh}}^{*} = \textbf{Re}(input[0]) \f$ \newline
- \f$ C_{2} = S_{hv}*S_{hv}}^{*} = \textbf{Re}(input[3]) \f$ \newline
- \f$ C_{3} = S_{vv}*S_{vv}}^{*} = \textbf{Re}(input[5]) \f$ \newline
- \f$ C_{4} = \textbf{Re}(S_{hh}*S_{hv}}^{*}) = \textbf{Re}(input[1]) \f$ \newline
- \f$ C_{5} = \textbf{Im}(S_{hh}*S_{hv}}^{*}) = \textbf{Im}(input[1]) \f$ \newline
- \f$ C_{6} = \textbf{Re}(S_{hh}*S_{vv}}^{*}) = \textbf{Re}(input[2]) \f$ \newline
- \f$ C_{7} = \textbf{Im}(S_{hh}*S_{vv}}^{*}) = \textbf{Im}(input[2]) \f$ \newline
- \f$ C_{8} = \textbf{Re}(S_{hv}*S_{vv}}^{*}) = \textbf{Re}(input[4]) \f$ \newline
- \f$ C_{9} = \textbf{Im}(S_{hv}*S_{vv}}^{*} = \textbf{Im}(input[4])) \f$ \newline
...
7. bsinclairtocoherency (SinclairToCoherencyMatrixFunctor) \newline
 ...
 - channel #3 : \f$ (S_{hh}+S_{vv}).(\textbf{i}(S_{hv}-S_{vh}))^{*} \f$ \newline
 - channel #4 : \f$ (S_{hh}-S_{vv}).(S_{hh}-S_{vv})^{*} \f$ \newline
 - channel #5 : \f$ (S_{hh}-S_{vv}).(S_{hv}+S_{vh})^{*} \f$ \newline
 - channel #6 : \f$ (S_{hh}-S_{vv}).(\textbf{i}(S_{hv}-S_{vh}))^{*} \f$ \newline
 - channel #7 : \f$ (S_{hv}+S_{vh}).(S_{hv}+S_{vh})^{*} \f$ \newline
 - channel #8 : \f$ (S_{hv}+S_{vh}).(\textbf{i}(S_{hv}-S_{vh}))^{*} \f$ \newline
 - channel #9 : \f$ \textbf{i}(S_{hv}-S_{vh}).(\textbf{i}(S_{hv}-S_{vh}))^{*} \f$ \newline
...
With: \newline
- \f$ S_{ll} = 0.5(-S_{hh}-\textbf{i}*S_{hv}-\textbf{i}*S_{vh}+S_{vv}) \f$ \newline
- \f$ S_{lr} = 0.5(-S_{hh}+\textbf{i}*S_{hv}-\textbf{i}*S_{vh}+S_{vv}) \f$ \newline
- \f$ S_{rl} = 0.5(-S_{hh}-\textbf{i}*S_{hv}+\textbf{i}*S_{vh}-S_{vv}) \f$ \newline
- \f$ S_{rr} = 0.5(-S_{hh}+\textbf{i}*S_{hv}+\textbf{i}*S_{vh}+S_{vv}) \f$ \newline
 
10. bsinclairtomueller (SinclairToMueller) \newline
- channel #0  : \f$ 0.5\textbf{Re}( T_{xx}.T_{xx}^{*} + T_{xy}.T_{xy}^{*} + T_{yx}.T_{yx}^{*} + T_{yy}.T_{yy}^{*} ) \f$ \newline
- channel #1  : \f$ 0.5\textbf{Re}( T_{xx}.T_{xx}^{*} - T_{xy}.T_{xy}^{*} + T_{yx}.T_{yx}^{*} - T_{yy}.T_{yy}^{*} ) \f$ \newline
- channel #2  : \f$ \textbf{Re}( T_{xx}.T_{xy}^{*} + T_{yx}.T_{yy}^{*} ) \f$ \newline
- channel #3  : \f$ \textbf{Im}( T_{xx}.T_{xy}^{*} + T_{yx}.T_{yy}^{*} ) \f$ \newline
- channel #4  : \f$ 0.5\textbf{Re}( T_{xx}.T_{xx}^{*} + T_{xy}.T_{xy}^{*} - T_{yx}.T_{yx}^{*} - T_{yy}.T_{yy}^{*} ) \f$ \newline
- channel #5  : \f$ 0.5\textbf{Re}( T_{xx}.T_{xx}^{*} - T_{xy}.T_{xy}^{*} - T_{yx}.T_{yx}^{*} + T_{yy}.T_{yy}^{*} ) \f$ \newline
- channel #6  : \f$ \textbf{Re}( T_{xx}.T_{xy}^{*} - T_{yx}.T_{yy}^{*} ) \f$ \newline
- channel #7  : \f$ \textbf{Im}( T_{xx}.T_{xy}^{*} - T_{yx}.T_{yy}^{*} ) \f$ \newline
- channel #8  : \f$ \textbf{Re}( T_{xx}.T_{yx}^{*} + T_{xy}.T_{yy}^{*} ) \f$ \newline
- channel #9  : \f$ \textbf{Im}( T_{xx}.T_{yx}^{*} - T_{xy}.T_{yy}^{*} ) \f$ \newline
- channel #10 : \f$ \textbf{Re}( T_{xx}.T_{yy}^{*} + T_{xy}.T_{yx}^{*} ) \f$ \newline
- channel #11 : \f$ \textbf{Im}( T_{xx}.T_{yy}^{*} - T_{xy}.T_{yx}^{*} ) \f$ \newline
- channel #12 : \f$ \textbf{Re}( T_{xx}.T_{yx}^{*} + T_{xy}.T_{yy}^{*} ) \f$ \newline
- channel #13 : \f$ \textbf{Im}( T_{xx}.T_{yx}^{*} - T_{xy}.T_{yy}^{*} ) \f$ \newline
- channel #14 : \f$ \textbf{Re}( T_{xx}.T_{yy}^{*} + T_{xy}.T_{yx}^{*} ) \f$ \newline
- channel #15 : \f$ \textbf{Im}( T_{xx}.T_{yy}^{*} - T_{xy}.T_{yx}^{*} ) \f$ \newline
...
- channel #3 : \f$ 0.5*(M_{13} + M_{23} + \textbf{i}*(M_{14}+M_{24})) \f$ \newline
- channel #4 : \f$ 0.5*(M_{33} - M_{44} - 2*\textbf{i}*M_{34}) \f$ \newline
- channel #5 : \f$ 0.5*(M_{13} - M_{23} - \textbf{i}*(M_{14}-M_{24})) \f$ \newline
