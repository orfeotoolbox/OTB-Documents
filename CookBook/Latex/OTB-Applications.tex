...
application corresponding to \verb?module_name? is. It is also 
possible to set this path with the environment variable 
\verb?ITK_AUTOLOAD_PATH?. This variable is checked by default when 
no \verb?[MODULEPATH]? argument is given (for instance, it can be
usefull to add the standard \otb installation directory to this 
variable). An error in the application name (i.e. in parameter
...
or \verb?ITK_AUTOLOAD_PATH?). 

Launching the \verb?otbApplicationLauncherCommandLine? with a valid
application name and no or incomplete parameters results will make the
