...
This section is specific to polarimetric datas.
...
of linear polarization (HH, HV, VH or VV). In polarimetry the basic 
measurement is a 2×2 complex scattering matrix yielding an eight dimensional 
measurement space (Sinclair matrix). For reciprocal targets where HV = VH, this space is 
compressed to five dimensions: three amplitudes (|HH|, |HV|, and |VV|); 
and two phase measurements, (co-pol: HH-VV, and cross-pol: HH-HV). (Explanation extracted from 
\href{ http://www.grss-ieee.org/technical-briefs/imaging-radar-polarimetry}{grss-ieee}).

\subsubsubsection{Synthesis}
...
The module waits for real and imaginary part (real images) of the HH, VV, VH and HV images.
The reciprocal case where case VH = HV, is not properly handled yet, the user
has to set the same input for the two HV and VH.

\subsubsubsection{Conversion}
As we saw in the previsou main section,  polarimetry the basic 
measurement is a 2×2 complex scattering matrix yielding an eight dimensional 
...
\item etc
\end{itemize}
The modules of the Conversion menu allows to proceed conversions between matrix representation.
The allowed conversion and their images types are described in the following figure ~\ref{fig:sarpolconv}.

\begin{figure}
  \center
  \includegraphics[width=0.6\textwidth]{../Art/MonteverdiImages/monteverdi_sarpol_conversion_schema.png}
  \itkcaption[SAR polarimetry conversion]{SAR polarimetry conversio module.}
