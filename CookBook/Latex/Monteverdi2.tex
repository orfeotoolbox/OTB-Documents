...
Installation of \mont2 is very simple. Standard installer packages are available on the main platforms thanks to OTB-Developpers and external users. These packages are available few days after the release. Get the latest information on binary packages on the \website in the section download.

We will discribe in the following sections the way to install \mont2 on:
\begin{itemize}
...
For Windows XP/Seven users, there is a classical standalone installation program for \mont2, available from the \download after each release of \mont2. 

\subsection{MacOS X}
A standard DMG package is available for \mont2 for MacOS X 10.8. Please go the \download.
Click on the file to launch \mont2. We will provide in the nex release a package for MacOSX 10.9.

\subsection{Ubuntu 12.04 and higher}
For Ubuntu 12.04 and higher, \mont2 package may be available as Debian package through APT repositories.

Since release 0.2, \mont2 packages are available in the
