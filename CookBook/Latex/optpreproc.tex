...
Orthorectification or map reprojection is generally not sufficient to provide
image registrations. Indeed, several sources of geometric
distortion can be contained in optical remote sensing images including earth
rotation, platform movement, nonlinearity\ldots 

As a result it is generally necessary to update image level. 
...
This figure illustrates the workflow to apply to register image series: 
%add figure
\begin{center}
...
Image superposition principle

\subsubsection{Extract metadata from the image reference}

We first dump geometry metadata of the image we want to refine. As you will see the application wich will estimate a refine geometry only needs as input this metadata and a set of homologous points.
...
The main idea of the residual registration is to estimate a residual transformation (after the application of sensors model) to allow to correct 

The basic idea is to use image pixels in both images to find a set of homologous points and estimate with them a residual transformation which will be use to superimpose. 
  
So the first step is to compute homologous points between images using keypoints. SIFT or SURF keypoints can be used and the band on which keypoints are computed can be set independantly for both images. 
The application offers two modes : the first is the full mode where keypoints are extracted from the full extent of both images (please note that in this mode large image file are not supported). The second mode, called \textit{geobins}, allows to set-up spatial binning so as to get fewer points spread accross the entire image. In this mode, the corresponding spatial bin in the second image is estimated using geographical transform or sensor modelling, and is padded according to the user defined precision. 
Last, in both modes the application can filter matches whose colocalisation in first image exceed this precision. The elevation parameters are to deal more precisely with sensor modelling in case of sensor geometry data. The \textit{outvector} option allows to create a vector file with segments corresponding to the localisation error between the matches. 
...
This application reads the input geometry metadata file which contains the sensor model of the image that we want to refine the geometry and a text file containing a list of ground control point, and performs a least-square fit of the sensor model adjustable parameters to these tie points. It produces an updated geometry file as output (the extension always use is \textit{.geom})

The application can provide as well as an optional ground control points based statistics file and a vector file containing residues. 
...
Now we will show how we can use this new sensor model. In our case we'll use this sensor model

\otb offers since version 3.16 the possibility to extend image path to use different metadata.

That's what we are going to use there to superimpose the second image over the first one using for the second image the estimated sensor model which take into account the original sensor model of the slave and which also fit to the set of tie points.
...
As a result, if you've got enough homologous points in images and control that the residual error between the set of tie points and the estimated sensor model is small, you must achieve a godd registration between the 2 images.
