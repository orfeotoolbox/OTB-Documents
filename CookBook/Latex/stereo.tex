...
We will now explain how to use the \application{StereoRectificationGridGenerator}
...
displacements. The \application{GridBasedImageResampling} allows to
...
the \application{StereoRectificationGridGenerator} application to set
...
\begin{figure}[!h]
  ...
  \itkcaption[ExampleEpipolar]{Extract of resample image1 and image2 in epipolar
    geometry over Pyramids of Cheops. \textit{\copyright CNES 2012}}
...
\subsection{Disparity estimation: Block matching along epipolar lines}
...
we will describe the power of the \application{BlockMatching}
application.
...
\application{StereoRectificationGridGenerator} application. Regarding the
...
\item -mask.inleft and -mask.inright: you can specify a no-data value which
  ...
  This mask can be easily generated using the \application{BandMath} application:
...
\subsection{From disparity to Digital Elevation Model}
...
the disparity map given by the \application{BlockMatching} application
...
This filtering can be easily done with \app.

We first use the \application{BandMath} application to filter
...
Then, we concatenate thresholded disparities using the \application{ConcatenateImages}:
...
\subsection{One application to rule them all}
An application has been added to combine all these steps using a standard approach 
: \application{StereoFramework}. It computes the DEM (still projected in WGS84) 
from an input stereo pair. The user has to specify the sampling step (in meters) 
of the output DEM, but the other parameters are optional. In addition to the 
usual elevation parameters, here are the main parameters to control the application:
\begin{itemize}
\item \code{-res} : sampling step of the output DEM in meters
\item \code{-above} : maximum elevation offset from the chosen elevation source (in meters)
\item \code{-below} : minimum elevation offset from the chosen elevation source (in meters)
\item \code{-mask.left} and \code{-mask.right} : input mask for the stereo pair
\item \code{-disp} : path to the output disparity map
\end{itemize}

The parameters \code{-above} and \code{-below} are used inside the application to derive the
minimum and maximum horizontal disparity exploration, so they have a critical impact on 
computation time. It is advised to choose an elevation source that is not too far from the
DEM you want to produce (for instance, an SRTM elevation model). Therefore, the altitude 
...
\begin{itemize}
\item Computation of epipolar rectification grids
\item Resampling into epipolar geometry with BCO interpolator
\item Block matching with NCC score on a 9x9 correlation window (only horizontal exploration)
\item Subpixel refinement with dichotomy method
\item Median filtering of horizontal disparities
\item Filtering of disparities with a low correlation (NCC lower than 0.6)
\item Projection on a WGS84 grid
\end{itemize}
