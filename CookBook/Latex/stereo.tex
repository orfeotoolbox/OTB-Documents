...
\application{StereoFramework}. It computes the DSM 
...
method, 
 \begin{itemize}
\item \code{-output.map} : output projection map selection.
\item \code{-output.res} : Spatial Sampling Distance of the output
DSM in meters
\item \code{-output.nodata} : DSM empty cells are filled with this float value
(-32768 by default)
\item \code{-output.fusionmethod} : Choice of fusion strategy in each DSM cell
(max, min, mean, acc) 
\item \code{-output.out} :  Output DSM
\item \code{-output.mode} : Output DSM extent choice
\end{itemize} 
\item[Stereorect :] Direct and inverse stereorectification grid subsampling
parameters
 \begin{itemize}
\item \code{-stereorect.fwdgridstep} : Step of the direct deformation grid (in
pixels)
\item \code{-stereorect.invgridssrate} : Sub-sampling of the inverse epipolar
grid
\end{itemize}
\item[BM : ] Block Matching parameters.
\begin{itemize}
 \item \code{-bm.metric} : Block-matching metric choice (robust SSD, SSD, NCC,
 Lp Norm) 
 \item \code{-bm.radius} : Radius of blocks for matching filter (in pixels,
 $2$ by default)
 \item \code{-bm.minhoffset} : Minimum altitude below the
 selected elevation source (in meters, -20.0 by default)
 \item \code{-bm.maxhoffset} : Maximum altitude above the
 selected elevation source (in meters, 20.0 by default)
\end{itemize}
\item[Postproc : ] Post-Processing parameters
 \begin{itemize}
 \item \code{-postproc.bij} : use bijection consistency. Right to Left
 correlation is computed to validate Left to Right disparities. If bijection is not found pixel is rejected
\item \code{-postproc.med} :  use median disparities filtering (disabled by
default)
\item \code{-postproc.metrict} : use block matching metric output to discard pixels with low correlation value (disabled by default, float value)");
\end{itemize}
\item[Mask : ] Compute optional intermediate masks.
\begin{itemize}
 \item \code{-mask.left} : Mask for left input image (must have the same size
 for all couples)
 \item \code{-mask.right} : Mask for right input image  (must have the same size
 for all couples)
 \item \code{-mask.variancet} : This parameter allows to discard pixels whose
...
The parameters \code{-bm.minhoffset} and \code{-bm.maxhoffset} are used
...
\code{-stereorect.fwdgridstep} and \code{-stereorect.invgridssrate} have also a
...
\application{ExtractROI} application on the other sensor images using the
fit mode \code{-mode.fit} option.
