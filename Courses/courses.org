...
**** Abstract
     ...
     classification model from Pleaides images and learning samples and how to
     apply this model to the input image and produce shiny classification map.
...
**** Produce and analize learning samples
     ...
     - What is the label corresponding to the class *water* i the shapefile?
       A set of learning samples is provided for the exercise in \textit{training.shp}
...
and standard deviation of 1) of samples based on the vector data produced by the user.
     ...
     - Use the *ComputeImagesStatistics* to compute statistics on \textit{melbourne_ms_toa_ortho_extract_small.tif} image
     - What is the mean of the red band?
     - The extract provided was firstly converted from DN to reflectance. For what reasons, is it advice to do such conversion for multiple images classification?
...
**** Apply classification model
     ...
     - Use the same model to apply the classification to the other extract \textit{melbourne_ms_toa_ortho_extract_large.tif}
...
** Solution 3: Supervised pixel based classification
*** Produce and analize learning samples
...
*** Estimate image statistics
    ...
    : $ ComputeImagesStatistics -il melbourne_ms_toa_ortho_extract_small.tif 
...
*** Estimate classification model using the Support Vector Machine algorithm
    ...
    : $ TrainSVMImagesClassifier -io.il melbourne_ms_toa_ortho_extract_small.tif 
    ...
    - The measured accuracy is 0.988523. The value is high sue to the low number of training samples and their lack of variability
*** Apply classification model
    
    - Here is the command line to produce the SVM model:

    : $ ImageSVMClassifier -in melbourne_ms_toa_ortho_extract_small.tif 
    : -svm melbourne_ms_toa_ortho_extract_small_model.svm 
    : -imstat melbourne_ms_toa_ortho_extract_small_stats.xml 
    : -out melbourne_ms_toa_ortho_extract_small_classification_5classes.tif uint8
*** Produce printable classification map
    We are going now to produce printable classification map using the
    *ColorMapping* application This tool will replace each label with an 8-bits
    RGB color specificied in a mapping file.  The mapping file should look like
    ...
    : $ ColorMapping -in melbourne_ms_toa_ortho_extract_small_classification_5classes.tif 
