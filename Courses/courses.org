...
**** From images to epipolar geometry :
     ...
     3. Use the application to generate 2 grids
...
**** Improve epipolar geometry Refinement of images geometry
     ...
     2. Open the 2 version of epipolar couples (total of 4 images). What differences do you notice between the two images pair?
...
**** Block matching :
     ...
     2. Propose manual or automatic methodologies to estimate the interval of disparities
...
**** Advanced Block matching : refine disparity map
     ...
     We are going to try here to improve the quality of disparity

     1. Use the Normalized Cross Correlation and output the metric value using the io.outmetric option. Open the metric image.Which value of correlation corresponds to a
     ...
     3. Use the *BandMath* application to only keep disparity with high metric value

**** From disparity map to elevation

     1. Use the *DisparityMapToElevationMap* to transform the disparity map into an elevation information to produce an elevation map
     2. At which height are approximatively football players on the ground in the stadium?
     3. What is approximatively the height of the stadium?
