...
**** Sentinel 2 - Level 2A Format
     ...
     Each Sentinel2-Level 2A product contains several *files*, which are classified as: 
         ...
         - FRE: (FRE for Flat REflectance) are also corrected for slope effect,
           which consists in suppressing the apparent reflectances
...
*** Chain OTB applications in-memory: exercise3.py
    ...
    5. In Application1, the output parameter has been declared with a filename.
       Has it been written as a file after the execution? Why?
       Solution: When ~ApplicationX.Execute()~ is used in the code, the 
    ...
    6. At the generation of the NDVI mask(with two possible values: water(1) and land(0) 
       ...
       Solution:
       In order to reduce the size of the output image without degrading the image, we may
       ...
       ~appX.SetParameterOutputImagePixelType("out","water_mask.tif?&gdal:co:NBITS=1")~
...
*** Water detection chain with NoData management: exercise4.py
    ...
    2. Open ~exercise4.py~ and complete the "FILL THE GAP 2" to set the BandMath expression
       to set the 255 value where the clouds image is different to zero, and otherwise
       keep the NDVI mask image.
       Solution : ~app3.SetParameterString("exp", "im2b1!=0?255:im1b1")~
    3. Launch ~exercise4.py~ with the different dates as arguments:
       ...
       and you will obtain three different masks. Open them with monteverdi to check
       the water extent variations.
    4. Look at the 20161218 final NDVI mask. What are the suspicious straight lines detected as water? 
       Solution: The straight lines are roads, which have similar spectral responses to water responses
...
*** Comparison with a reference: exercise5.py 
    ...
    1. Open the image ~ref/occurrence_90W_0N.tif~ in QGIS or Monteverdi. What values do you observe
       ...
       Solution: The value = 90 means that water has been found in the 90% of the observations during the 32 years
    ...
    3. Now it's time to compare each of the water masks of exercise 4 (on 3 dates: 20161218,
       ...
       for each of the 3 water masks.You may launch the otb application in the commandline: 
           otbcli-XXXXX -in XXXX -out XXXX -param1 XXXX          
       Solution:
       The ComputeConfusionMatrix application can be used to compare raster images as follows: 
       ~otbcli_ComputeConfusionMatrix -in GSW_10.tif -ref raster -ref.raster.in water_mask_SENTINEL2A_2017 -ref.raster.nodata "255" -out conf_A_90.csv
water_mask_SENTINEL2A_20170407-154054-255_L2A_T17MNP_D_V1-4.tif  water_mask_SENTINEL2A_20171203-154308-349_L2A_T17MNP_D_V1-4.tif~
    ...
    4. What would you conclude about the results?
       Solution: 
       The first image (the dryest one) represents a water extent surface that is seen at least 75% of the times.
       The second image have a water extent that has been seen only 10% of the times and the third one only the 20% of the times. 
       We may conclude that the second and third water extents are rare events given the low probabilities of flood. 
