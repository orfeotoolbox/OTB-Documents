...
In order to get useful results from this large scale segmentation, we need to
address several issues. The first issue is the splitting of regions on the
tiles' borders. The naive solution we implemented is a simple stitching rule: we
post-process the vector data by looking for neighbor polygons lying on each side
of a tile border and merge them if their contact surface is large enough. More
...
pre-processing as well as post-processing steps. Firstly, we added an input
...
segmentation output compatible with GIS software.
...
will include new stitching and tiling strategies to enhance the
segmentation performance at tiles borders, using approaches such as
presented in~\cite{crisp2003fast}. Moreover the computation of attributes
...
missing, we also hope to provide a comprehensive environment for
